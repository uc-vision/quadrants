...
`qd.Matrix` and `qd.Vector` values nested inside template or compound-container arguments are not eligible for
fastcache. Pass them as directly declared runtime matrix arguments instead.
